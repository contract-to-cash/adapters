...
// PostgresInvoiceRepository implements invoice.Repository on top of pgx.
//
// All persistence goes through the core ToSnapshot / InvoiceFromSnapshot API:
// Save writes the full snapshot in one upsert, FindBy* read into a snapshot
// and call InvoiceFromSnapshot to reconstitute the entity. Monetary values
// are stored twice — once as a BIGINT column for indexing/querying and once
// as JSONB for lossless round-trip — so that non-JPY currencies survive
...
// NewInvoiceRepository creates a new PostgresInvoiceRepository.
...
// invoiceJSONState is everything from the snapshot that either cannot be
// represented as a single SQL column, or that we want to persist losslessly
// as JSON alongside the int64 / timestamp columns used for indexing.
...
// Save upserts the invoice and appends a history row for FindByIDAsOf.
//
// Note: the two writes (invoices + invoice_history) must be atomic. Callers
// that invoke Save through TxManager.RunInTx get this for free; callers that
// call Save directly on the pool are responsible for opening their own
// transaction.
func (r *PostgresInvoiceRepository) Save(ctx context.Context, inv *invoice.Invoice) error {
	...
	_, err = q.Exec(ctx,
		...
		ON CONFLICT (id) DO UPDATE SET
			...
			applied_balance      = EXCLUDED.applied_balance,
	...
	// Close any open history row and append a new one for temporal queries.
	if _, err := q.Exec(ctx,
...
// FindByContractID loads all invoices for a contract.
func (r *PostgresInvoiceRepository) FindByContractID(ctx context.Context, contractID shared.ContractID) ([]*invoice.Invoice, error) {
	rows, err := r.q(ctx).Query(ctx,
		selectInvoiceSQL+` WHERE contract_id = $1 ORDER BY created_at DESC`,
		string(contractID),
	)
	if err != nil {
...
// FindOverdue returns invoices in "issued" state whose due_date has passed.
func (r *PostgresInvoiceRepository) FindOverdue(ctx context.Context) ([]*invoice.Invoice, error) {
	rows, err := r.q(ctx).Query(ctx,
		selectInvoiceSQL+` WHERE status = 'issued' AND due_date < NOW() ORDER BY due_date ASC`,
	)
	if err != nil {
...
// FindByIDAsOf returns the invoice state as it was at asOf, using
// invoice_history as a temporal table.
...
// FindByContractAndStatus returns invoices for a contract with the given status.
...
// FindByContractAndPeriod returns invoices whose issue_date falls within the
// given period.
...
// FindUnpaidByContract returns invoices for a contract in any non-settled status.
...
// scanTarget unifies pgx.Row and pgx.Rows for the shared scanner.
...
func scanInvoiceSnapshot(t scanTarget) (invoice.InvoiceSnapshot, error) {
	...
	// The canonical source of monetary values is the JSONB state; the int64
	// columns are a query-side denormalization and may be lossy for non-JPY
	// currencies.
