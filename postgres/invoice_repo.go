...
func (r *PostgresInvoiceRepository) Save(ctx context.Context, inv *invoice.Invoice) error {
	...
	if err := r.saveOn(txCtx, inv); err != nil {
...
func (r *PostgresInvoiceRepository) saveOn(ctx context.Context, inv *invoice.Invoice) error {
	s := inv.ToSnapshot()
...
func (r *PostgresInvoiceRepository) FindByContractAndPeriod(ctx context.Context, contractID shared.ContractID, period shared.DateRange) ([]*invoice.Invoice, error) {
	...
	// Zero-period invoices store NULL billing_period_from/to and never match a
	// non-zero query period (= NULL is never true), matching the reference.
	rows, err := r.q(ctx).Query(ctx,
		selectInvoiceSQL+` WHERE contract_id = $1 AND billing_period_from = $2 AND billing_period_to = $3 ORDER BY issue_date ASC`,
