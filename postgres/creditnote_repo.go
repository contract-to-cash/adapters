...
//
// CreditNote is not part of tx.Repos (core's TxManager contract) so callers
// construct this repository independently and call its methods outside
// RunInTx. Internal state mutations remain serialized by the pgx pool; the
// guarantee of atomicity across multiple credit notes is the caller's
// responsibility.
...
// NewCreditNoteRepository creates a new PostgresCreditNoteRepository.
...
// creditNoteJSONState is the lossless monetary + line-item payload of a
// credit note, mirrored from the snapshot.
...
func (r *PostgresCreditNoteRepository) Save(ctx context.Context, cn *invoice.CreditNote) error {
	...
	_, err = r.q(ctx).Exec(ctx,
		`INSERT INTO credit_notes (
			id, number, invoice_id, contract_id, account_id, status, reason, memo,
			items, subtotal, tax_amount, total, credit_amount, refund_amount,
			currency, issued_at, updated_at
		) VALUES (
...
// FindByInvoiceID returns credit notes for the given invoice.
...
// FindByAccountID returns credit notes for the given account.
...
// FindByContractID returns credit notes for the given contract.
...
// FindByStatus returns credit notes in the given status.
